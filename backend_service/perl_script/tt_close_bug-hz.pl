...
if ( $select ne "q" ) {
    ...
    if($project eq "AsusEmail") {
       $url_tt = $url_base . 'tmtrack.dll?ReportPage&Template=reports%2Flist&drilldownrptid=26166&incsub=1&projectid=5&tableid=1001&target=DrillDown&where=%28USR_BUG.TS_MANAGER%20in%20%281457%29%20and%20USR_BUG.TS_STATE%20in%20%285%2c2%2c7%29%20and%20USR_BUG.TS_GATTING_REQUIREMENT%20in%20%28284%2c3602%29%29';   
       #$url_tt = $url_base.'tmtrack.dll?ReportPage&Template=reports%2Flist&reportid=27124';     
    }
    $mech->get($url_tt);
    ...
    if ( $select eq "-9" ) {
        ...
        my $path_tag = $path_project . '/tags_format_' . $project . '_vendor_format_multi.csv';
        ...
        foreach my $key ( keys %hash_tt ) {
            if ( $hash_tt{$key} eq '1'  || $key eq '398982') {
                next;
            }
        ...
        foreach (@path) {
            if (/id=\"F1112\".*(ChangeLink.*?)<.*?(APK Version.*?)<.*?Add your comment/) {
                foreach my $key ( keys %hash_fill ) {
                    ...
                    # print $1.", ".$2."<br>";
                    if ( $1 . "\n" . $2 eq $hash_fill{$key} ) {
                        #print "Yep, Repeat Update - Do nothing : $1\n$2\n\n";
                        $flag = 0;
                    }
                }
...
sub getWebHash() {
    ...
    foreach my $table ( $te->tables ) {
        ...
        $value =~ s/\s\(Bug\)//; #For AsusEmail: A80 (Bug)
